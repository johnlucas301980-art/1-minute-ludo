 ...
 * Returns null when no match is found.
 */
export async function findByReferralCode(referralCode: string): Promise<UserRow | null> {
  if (!pool) return null;
  const { rows } = await pool.query<UserRow>(
    "SELECT * FROM users WHERE upper(referral_code) = upper($1) LIMIT 1",
    [referralCode],
  );
  return rows[0] ?? null;
}
 ...
 * `player_id` and `referral_code` are auto-generated by database triggers.
...
export async function createUser(
  ...
  const { rows } = await pool.query<Pick<UserRow, "id" | "player_id" | "full_name" | "email" | "mobile" | "status" | "created_at">>(
    `INSERT INTO users (full_name, email, mobile, password_hash, country, referred_by)
     VALUES ($1, $2, $3, $4, $5, $6)
